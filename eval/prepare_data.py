...
def prepare_benchmark_data(name, total_tokens, dim, bits=3):
    ...
    codebooks_flat = {f"cb_l{l}_centroids": cb[0].cpu().numpy() for l, cb in enumerate(tq.polar_quantizer.codebooks)}
    
    np.savez(config_path, 
             Pi=tq.polar_quantizer.Pi.cpu().numpy(),
    ...
    for i in range(0, total_tokens, shard_size):
        ...
        packed_angles = torch.cat([a.flatten() for a in q_data.polar_q.angle_indices]).cpu().numpy().astype(np.uint8)
        packed_angles.tofile(f_packed)
        ...
        # Thêm lưu TRỤC BÁN KÍNH CUỐI (final_radius)
        f_fr = open(f"{tq_dir}/tq_final_radius_{dim}.npy", "ab" if i > 0 else "wb")
        q_data.polar_q.final_radius.cpu().numpy().astype(np.float32).tofile(f_fr)
        f_fr.close()
