...
def run_stress_test_persistence():
    ...
    BATCH_SIZE = 100_000 # Streaming batch
    LAYERS = 8 # Simulating multiple layers or searches
    
    print(f"🔥 Starting Extreme Persistence Benchmark (1M, D={DIM})")
    ...
    for layer in range(LAYERS):
        ...
        for i in range(0, TOTAL_TOKENS, BATCH_SIZE):
            shard = torch.from_numpy(raw_mmap[i:i+BATCH_SIZE])
        ...
        print(f"  Layer {layer+1}/{LAYERS} processed.")
    ...
    for layer in range(LAYERS):
        ...
        for i in range(0, TOTAL_TOKENS, BATCH_SIZE):
            mse_packed = tq_packed_mmap[i:i+BATCH_SIZE][np.newaxis, ...]
            qjl_signs = tq_signs_mmap[i:i+BATCH_SIZE][np.newaxis, ...]
            norms = tq_norms_mmap[i:i+BATCH_SIZE][np.newaxis, ...]
            res_norms = tq_res_norms_mmap[i:i+BATCH_SIZE][np.newaxis, ...]
    ...
    avg_time_raw = time_raw / LAYERS
    avg_time_tq = time_tq / LAYERS
    ...
    print("\n" + "="*40)
    print("🏆 FINAL PERSISTENCE RESULTS (Average per Search)")
    print("="*40)
    print(f"RAW (FP32)  | Peak RAM: {actual_peak_raw:.2f} MB | Avg Time: {avg_time_raw:.4f}s")
    print(f"TQ (3-bit)  | Peak RAM: {actual_peak_tq:.2f} MB | Avg Time: {avg_time_tq:.4f}s")
    print("-" * 40)
