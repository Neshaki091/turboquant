...
def run_master_analyzer():
    ...
    BATCH_CONFIG = {
        "RAW": 200_000 if is_optimal else 500_000,
    ...
    for label, bits in TQ_VARIANTS:
        ...
        Pi, S = torch.from_numpy(conf['Pi']), torch.from_numpy(conf['S'])
        ...
        f_fr = open(f"{tq_dir}/tq_final_radius_768.npy", "rb")
        
        # Đọc động kích thước file để lách qua phần tính count phức tạp của Polar
        packed_bytes_per_token = os.path.getsize(f"{tq_dir}/tq_packed_768.npy") // TOTAL_TOKENS
        signs_bytes_per_token = os.path.getsize(f"{tq_dir}/tq_signs_768.npy") // TOTAL_TOKENS
        fr_bytes_per_token = os.path.getsize(f"{tq_dir}/tq_final_radius_768.npy") // TOTAL_TOKENS // 4 # (float32=4 bytes)
        
        tq_start = time.perf_counter()
        peak_tq = get_current_memory_mb()
        
        for _ in range(N_QUERIES):
            f_p.seek(0); f_s.seek(0); f_n.seek(0); f_r.seek(0); f_fr.seek(0)
            query = torch.randn(1, DIM)
            ...
            for i in range(0, TOTAL_TOKENS, b_size):
                ...
                packed = np.fromfile(f_p, dtype='uint8', count=curr * packed_bytes_per_token)
                signs = np.fromfile(f_s, dtype='uint8', count=curr * signs_bytes_per_token)
                norms = np.fromfile(f_n, dtype='float32', count=curr)
                res_n = np.fromfile(f_r, dtype='float32', count=curr)
                fr = np.fromfile(f_fr, dtype='float32', count=curr * fr_bytes_per_token)
                
                # Giả lập tính toán GPU/SIMD score trên dữ liệu nén
                peak_tq = max(peak_tq, get_current_memory_mb())
                del packed, signs, norms, res_n, fr
                if i % 1_000_000 == 0: gc.collect(); empty_windows_working_set()
        ...
        for f in [f_p, f_s, f_n, f_r, f_fr]: f.close()
